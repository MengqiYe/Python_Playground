...
def process_odds(in_queue, shared_list):
    ...
    while True:
        ...
        if new_value == POISON_PILL:
            break
        print(f"process_odds : {new_value}")
...
def process_evens(in_queue, shared_list):
    ...
    while True:
        ...
        if new_value == POISON_PILL:
            break
        print(f"process_evens : {new_value}")
        shared_list.append(new_value/-2)

    return

def input_loop(odd_queue, even_queue):
    is_odd = True
    x = 0
    while True:
        if is_odd:
...
def main():
    ...
    t = threading.Thread(target=input_loop, args=(odd_queue, even_queue))
    t.start()
